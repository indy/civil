...
function ExpandableListingLink({ index, resource, deckId, deckName, deckInsignia, deckLevelRefs, deckLevelAnnotation, notes, expanded, onExpandClick }: { index?: any, resource?: any, deckId?: any, deckName?: any, deckInsignia?: any, deckLevelRefs?: any, deckLevelAnnotation?: any, notes?: any, expanded?: any, onExpandClick?: any }) {
...
function buildNotes(notes) {
    let res = notes.reduce((a, note) => {
        if (note.topAnnotation) {
            a.push((<div class="ref-top-scribble">{ note.topAnnotation }</div>));
        }
        let refs = note.refs && note.refs.map(r => {
            return (<Ref deckReference={r} extraClasses="left-margin-entry"/>);
        });

        a.push(<div class="note">
