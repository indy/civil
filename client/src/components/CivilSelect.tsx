...
import { svgCloseShifted } from '../svgIcons';
import { sortByResourceThenName, indexToShortcut } from '../CivilUtils';
...
function candidateToRef(candidate: any) {
    return {
        id: candidate.id,
        name: candidate.name,
...
function rebuildCurrentSelection(state: any) {
...
export default function CivilSelect({ parentDeckId, chosen, onFinish }: { parentDeckId?: any, chosen?: any, onFinish?: any }) {
    ...
    <div class='civsel-main-box'>
        ...
        <button onClick={ onLocalCommit } disabled={ !local.canSave }>{ local.showKeyboardShortcuts && `Ctrl-Enter`} Save Changes</button>
    </div>);
}

function SelectedReference({ reference, onRemove, onChangeKind, onChangeAnnotation }: { reference?: any, onRemove?: any, onChangeKind?: any, onChangeAnnotation?: any }) {
    ...
    let topclass = `civsel-reference pigment-${reference.resource}`;
...
function Input({ text, onTextChanged, onAdd, onCreate, candidates, currentSelection, showKeyboardShortcuts }: { text?: any, onTextChanged?: any, onAdd?: any, onCreate?: any, candidates?: any, currentSelection?: any, showKeyboardShortcuts?: any }) {
...
function CandidateItem({ candidate, onSelectedCandidate, showKeyboardShortcuts, keyIndex }: { candidate?: any, onSelectedCandidate?: any, showKeyboardShortcuts?: any, keyIndex?: any }) {
    ...
    const topclass = `civsel-candidate pigment-${candidate.resource}`;
    return (
        <div class={topclass} onClick={selectedThisCandidate}>
        { canShowKeyboardShortcut && (<span class='keyboard-shortcut'>{ indexToShortcut(keyIndex)}: </span>)}
        { candidate.name }
    </div>);
}
