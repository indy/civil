...
export default function buildMarkup(content: string) {
    ...
    function assignPseudoParagraph(klass?: any) {
        let c = klass || "";
        c += " pseudo-paragraph";
        return c.trim();
    }

    function compile(n?: any, withinRightMargin?: any) {
        ...
        if (n.name === "text") {
            ...
            //
            return h(Image, attrs(n), ...n.children.map(child => { return compile(child, withinRightMargin)}));
        } else {
            return h(n.name, attrs(n), ...n.children.map(child => { return compile(child, withinRightMargin)}));
        }
    }
