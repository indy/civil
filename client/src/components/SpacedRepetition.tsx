...
export default function SpacedRepetition({ path }: { path?: string }) {
    ...
    if (local.mode === MODE_PRE_TEST && !canTest) {
        const nextReviewDate = formattedDate((Date.parse(appState.srEarliestReviewDate.value!) as unknown) as string);
        const nextReviewTime = formattedTime((Date.parse(appState.srEarliestReviewDate.value!) as unknown) as string);
...
//
function CardTest({ card, onRatedCard, onShowAnswer }: { card?: any, onRatedCard?: any, onShowAnswer?: any }) {
    ...
    <div>
        ...
        <div class="note">
            <div class="left-margin">
                ...
                <Link class={klass} href={ href }>{ name }</Link>
                </div>
            </div>
