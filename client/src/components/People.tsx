...
import { IPerson, IDeckSimple, ISearchResults, IPeopleListings } from '../types';
...
import { svgPointAdd,
         ...
         svgBlank,
         svgTickedCheckBox,
         svgUntickedCheckBox } from '../svgIcons';
...
function Person({ path, id }: { path?: string, id?: string }) {
    ...
    useEffect(() => {
        Net.get<ISearchResults>(`/api/people/{id}/additional_search`).then(searchResults => {
    ...
    <article>
        ...
        { name && !hasKnownLifespan && <LifespanForm name={ name } onLifespanGiven={ onLifespan } oldestAliveAge={ appState.oldestAliveAge }/> }
...
function preCacheFn(person?: any) {
    ...
    function addAge(point?: any, born?: any) {
        ...
        let eventTriple: [number, number, number] = dateStringAsTriple(point.date)!;
...
function SectionUpdatePerson({ person, onUpdate }: { person?: any, onUpdate?: any }) {
...
function PersonDeckPoint({ deckPoint, hasNotes, noteManager, holderId }: { deckPoint?: any, hasNotes?: any, noteManager?: any, holderId?: any }) {
...
function ListDeckPoints({ deckPoints, deckManager, holderId, holderName, showAddPointForm }: { deckPoints?: any, deckManager?: any, holderId?: any, holderName?: any, showAddPointForm?: any }) {
    ...
    function onAddDeathPoint(point: any) {
        Net.post<any, IPerson>(`/api/people/{holderId}/points`, point).then(person => {
