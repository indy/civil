...
fn from_rusqlite_row(row: &Row) -> rusqlite::Result<Concept> {
        Ok(Concept {
            id: row.get(0)?,
            title: row.get(1)?,
            deck_kind: row.get(2)?,
...
fn get_or_create_conn(
    ...
    concept.notes = notes_db::notes_for_deck_conn(conn, concept.id)?;
    concept.arrivals = notes_db::arrivals_for_deck_conn(conn, concept.id)?;
...
fn convert_conn(
    ...
    let mut concept = conn.prepare_cached(DECKBASE_QUERY)?
        .query_row(params![user_id, concept_id, DeckKind::Concept.to_string()], |row| from_rusqlite_row(row))
        .optional()?;

    if let Some(ref mut i) = concept {
        i.notes = notes_db::notes_for_deck_conn(conn, concept_id)?;
        i.arrivals = notes_db::arrivals_for_deck_conn(conn, concept_id)?;
    }
...
//
pub(crate) async fn convert(sqlite_pool: &SqlitePool, user_id: Key, concept_id: Key) -> crate::Result<Option<Concept>> {
    db(sqlite_pool, move |conn| convert_conn(conn, user_id, concept_id))
        .await
        .map_err(Into::into)
}
...
fn get_conn(
    ...
    decks::hit_conn(&conn, concept_id)?;

    Ok(concept)
}

pub(crate) async fn get(sqlite_pool: &SqlitePool, user_id: Key, concept_id: Key) -> crate::Result<Option<Concept>> {
...
fn edit_conn(
    ...
    concept.notes = notes_db::notes_for_deck_conn(conn, concept_id)?;
    concept.arrivals = notes_db::arrivals_for_deck_conn(conn, concept_id)?;
...
pub(crate) async fn edit(
    ...
    db(sqlite_pool, move |conn| edit_conn(conn, user_id, concept, concept_id))
        .await
        .map_err(Into::into)
}
...
fn delete_conn(
    ...
    decks::delete_conn(&conn, user_id, concept_id)?;

    Ok(())
}

pub(crate) async fn delete(sqlite_pool: &SqlitePool, user_id: Key, concept_id: Key) -> crate::Result<()> {
    db(sqlite_pool, move |conn| delete_conn(conn, user_id, concept_id))
        .await
        .map_err(Into::into)
}
