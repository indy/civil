...
pub async fn create_entry(
    ...
    Ok(HttpResponse::Ok().json(web::Entry::from(db_entry)))
}
...
pub async fn get_entries(
    ...
    let entries: Vec<web::Entry> = db_entries
        .into_iter()
        .map(|db_entry| web::Entry::from(db_entry))
        .collect();
...
mod db {
    ...
    impl From<Entry> for web::Entry {
        fn from(e: Entry) -> web::Entry {
            web::Entry {
                id: e.id,
                content: e.content,
            }
    ...
    pub async fn create_entry(db_pool: &Pool, entry: &web::Entry, user_id: i64) -> Result<Entry> {
